...
class FusionCriterion(Function):
    def __init__(self, regWeight, varWeight):
        ...
        self.skeletonRef = [[[3,1], [4,1]]]
        self.skeletonWeight = [[1, 1]]


    def forward(self, input, target_):
        target = target_.view(target_.size(0), 5, 3)
        xy = target[:, :, :2]
        z = target[:, :, 2]
        ...
        for t in range(batchSize):
            ...
            if s < ref.eps and s > - ref.eps: #Sup data
                loss = (((input[t] - z[t]) ** 2).sum() / 5).float()
                output += float(self.regWeight * loss)
            else:
    ...
    def backward(self, grad_output):
        ...
        target = target.view(target.size(0), 5, 3)
        xy = target[:, :, :2]
        z = target[:, :, 2]
        grad_input = torch.zeros(input.size(),dtype=torch.float)
        batchSize = target.size(0)
        for t in range(batchSize):
            s = xy[t].sum()
            if s < ref.eps and s > - ref.eps:
                grad_input[t] += (grad_output[0].cpu() * self.regWeight / batchSize * 2 / 5 * (input[t].cpu() - z[t].cpu())).cpu()
            else:
                xy[t] = 2.0 * xy[t] / ref.outputRes - 1
                for g in range(len(self.skeletonRef)):
                    E, num = 0, 0
                    ...
                    for j in range(N):
                        if l[j] > 0:
                            id1, id2 = self.skeletonRef[g][j]
                            grad_input[t][id1] += float(2 * self.varWeight * self.skeletonWeight[g][j] ** 2 / num * (l[j] - E) / l[j] * (input[t, id1] - input[t, id2]) / batchSize)
                            grad_input[t][id2] += float(2 * self.varWeight * self.skeletonWeight[g][j] ** 2 / num * (l[j] - E) / l[j] * (input[t, id2] - input[t, id1]) / batchSize)
