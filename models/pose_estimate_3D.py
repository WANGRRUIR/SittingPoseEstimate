...
class StackedHourgalss3D(nn.Module):
    def __init__(self,stack_num,residual_num,channel_num,res_num):
        ...
        for i in range(stack_num):
            ...
            _fc.append(fc)
            _out.append(nn.Conv2d(self.channel_num,5,bias=True,kernel_size=1,stride=1))
            _inner_fc.append(nn.Conv2d(self.channel_num,self.channel_num,bias=True,kernel_size=1,stride=1))
            _inner_out.append(nn.Conv2d(5,self.channel_num,bias=True,kernel_size=1,stride=1))
        ...
        self.reg=nn.Linear(2*2*self.channel_num,5)
