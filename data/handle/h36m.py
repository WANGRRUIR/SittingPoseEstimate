...
from data.handle.img import Crop, DrawGaussian, Transform3D
...
class H36M(data.Dataset):
    ...
    def LoadImage(self, index):
        folder = 's_{:02d}_act_{:02d}_subact_{:02d}_ca_{:02d}'.format(self.annot['subject'][index], self.annot['action'][index],self.annot['subaction'][index], self.annot['camera'][index])
        path = '{}/{}/{}_{:06d}.jpg'.format(ref.h36m_img_dir, folder, folder, self.annot['id'][index])
        #print(path)
        #print 'path', path
        img = cv2.imread(path)
        return img
    ...
    def __getitem__(self, index):
        ...
        #sitting
        outMap=outMap[[6,8,9,12,13],...]
        outReg=outReg[[6,8,9,12,13],...]
        pts_3d_mono=pts_3d_mono[[6,8,9,12,13],...]
